...
**Aufgabe**: Inspizieren Sie die Datei mit einem Text-Viewer und/oder mit Excel.

Wir laden die Tabelle wie folgt:

```{r}
read_delim( "data_on_git/GFP_object_count_perImage20221020withIL2.txt", 
...
Hinweis: Die Aufteilung der Daten auf 4x2 "Facets" ("Trellis-Plot" oder "facets plot") erreichen
Sie durch hinzufügen von `facet_grid( n0_t_cells ~ subject )` zu Ihrem
ggplot-Befehl. Vor der Tilde steht hier die Tabellenspalte, die die Facetten-Zeile
bestimmt (also, ob der Plot in der oberen Reihe (12000 Zellen) oder in der
unteren Reihe (24000 Zellen) steht), und nachd er Tilde die Tabellenspalte
für die Facetten-Spalte.
