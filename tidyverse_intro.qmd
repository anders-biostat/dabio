...
Ein Teilpaket von Tidyverse ist `ggplot2`, das Fuktionalität zur Daten-Visualisierung bereit stellt. Das folgende Beispiel erzeugt ein Streudiagramm (*scatter plot*) der Anzahl der Morde gegen die Einwohnerzahl:
...
Das kann man (s.o.) auch so schreiben:
```r
tabelle %>% mutate( spaltenname = ausdruck )
```

Beides fügt in der Tabelle `tabelle` eine Spalte mit Namen `spaltenname` hinzu und füllt die neue Spalte mit dem Wert von `ausdruck`. Dazu muss `ausdruck` ein Vektor sein mit einem Element für jede Tabellenzeile. Alternativ kann `ausdruck` auch ein einzelner Wert (Skalar) sein, der dann in jeder Zeile wiederholt wird. Im `ausdruck` kann man die Namen existierender Spalten verwenden, um mit ihren Werten zu rechnen.
...
- Entfernen Sie alel Zeilen zu Staaten, die weniger als 1 Million Einwohner enthalten.
...
## Tidverse-Pipes: Zusammenfassung

Hier noch einmal die wichtigsten Regel für Tidyverse:

- Jede Tidyverse-Operation benötigt ein *Tidyverse-Verb*. Bisher kennen wir die Tidyverse-Verben
  `mutate`, `arrange`, `select`, `filter`. Später werden wir noch `group_by`, `summarize`,
  `pivot_wider`, `pivot_longer` und einige weitere Verben kennen lernen.
- Alle Tidyverse-Operation erwarten eine Tabelle (tibble) als Eingabe und liefern eine veränderte Tabelle als
  Ausgabe.
- Mehrere Operation können als "Pipeline" verkettet werden. Dabei *muss* jeder Schritt 
  durch Verb beschrieben sein. Jeder Schritt nimmt die Tabelle vom vorherigen Schritt und gibt
  die veränderte Tabelle an den nächsten Schritt weiter.
- Das erste Argument jedes Verbs muss die eingehende Tabelle sein. Man kann diese entweder 
  - gleich hinter die öffnende Klammer schreiben: `verb( tabelle, ... )`, oder
  - mit dem Pipe-Symbol `%>%` von links hinein schieben: `tabelle %>% verb( ... )`
- Tidyverse-Verben geben stets eine Tabelle zurück. Diese kann man entweder
  - zu einem weiteren Verb weiter schieben: `tabelle %>% verb1( ... ) %>% verb2( ... )`, oder
  - als Ergebnis in eine Variable speichern: `tabelle %>% verb1( ... ) -> neue_tabelle`, oder
  - als Ergebnis auf der Konsole ausgeben lassen: `tabelle %>% verb1( ... )`
- Wichtig: Wenn eine Tabelle auf der Konsole ausgegeben wird, wird sie verworfen und steht *nicht*
  weiter zur Verfügung. Wenn die Tabelle in eine Variable gespeichert wird, wird sie *nicht* ausgegeben.
  Wenn man beides möchte, muss man die Tabelle erst in eine Variable speichern, und dann den Inhalt
  dieser Variable als neuen Schritt ausgeben lassen:
```
tabelle %>% verb( ... ) -> neue_tabelle
neue_tabelle
```
...
Beispiel-Daten aus Kursen sind oft als Paket verfügbar, wie z.B. das `dslab`-Paket von oben. Wenn man das Paket
lädt, dann leigt die Tabelle als Variable vor.

### Direkte Eingabe

Wir können die Daten auch direkt eintippen, und dann daraus eine Tabelle erstellen.

Beispiel: Als Teil einer ökologischen Feldstudie soll untersucht werden, ob die Wühlmäuse in Waldgebiet "Südwald" besser genährt sind als im "Ostforst". Dazu werden in beiden Waldgebieten Lebendfallen aufgestellt. Die gefangenen Mäuse werden gewogen und wieder frei gelassen.
...
Diese Datei können wir dann z.B. in Excel laden. Umgekehrt können wir Excel-Tabellen im CSV-Format abspeichern und in R mit `read_csv` laden.

#### Aufgabe

Sie waren im Wald, haben ein Zählquadrat abgesteckt und darin alle Käfer gezählt. Sie haben 3 Hirschkäfer, 12 Rüsselkäfer, 5 Hornkäfer, und 4 Marienkäfer und einige weitere gefunden. Erstellen Sie in Excel (oder einem anderen Tabellenkalkulations-Programm) eine Tabelle mit zwei Spalten mit den Überschriften "Spezies" und "Anzahl". Schreiben Sie darunter die genannten Zahlen. Speichern sie die Tabelle im CSV-Format (!) ab.

Laden Sie die Tabelle in R mit `read_csv( dateiname )`. Alternativ können Sie auch in der "Environment"-Pane die Schaltfläche "Import Dataset" verwenden. Legen Sie die Tabelle in eine Variable `kaefer` ab.

Erstellen Sie ein Balkendiagramm mit ggplot und `geom_col` gemäß der Anleitung oben. Übergeben Sie `geom_col` die Spalte `Spezies` für die `x`-Werte und die Spalte `Anzahl` für die `y`-Werte.
...
- Was ist die durschnittliche Körpergröße eines erwachsenen Mannes und einer erwachsenen Frau? (Nutzen Sie `filter`, um alle minderjährigen Probanden (mit Alter unter 18) aus der Tabelle zu entfernen, gruppieren Sie dann nach Geschlecht und berechnen Sie dann die Mittelwerte von `height` in den beiden Gruppen.)
