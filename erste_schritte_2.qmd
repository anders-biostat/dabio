...
Falls `install.packages` mit der Fehlermeldung "internet routines cannot be 
loaded" abbricht, dann geben Sie folgenden Befehl ein und versuchen es danach nochmal:
...
*Hinweis*: Der Zugriff auf Tabellen-Spalten mit `$` ist "Old-School"-R. Im nächsten Kapitel werden wir
"Tidyverse" benutzen, das anders mit Tabellen umgeht. Wenn Sie mit Tidyverse arbeiten, brauchen Sie
den `$`-Operator normalerweise nicht.
