...
Laden Sie die Tabellen DEMO_J und BMX_J von der NHANES-Webseite herunter ([Durchgang 2017/18](https://wwwn.cdc.gov/nchs/nhanes/continuousnhanes/default.aspx?BeginYear=2017), unter "Demographics data" und "Examination data").

(Hinweis: Manche Web-Browser laden die Datei nicht herunter, wenn man den Link anklickt, sondern versuchen, sie im Browserfenster darzustellen, was nur zu wirren Zeichen führt. In diesem Fall klicken Sie mit der *rechten* Maustaste auf den Link. Dann erscheint das Kontextmenü, in dem die Funktion zum Abspeichern beschriftet ist mit "Link speichern unter", "Save link as", "Ziel speichern unter", "Verknüpfte Datei laden" oder ähnlichem. Bei MacBooks ohne rechte Maustaste müssen Sie beim Klicken die Strg-Taste gedrückt halten, um das Kontextmenü zu erhalten.)

Fügen Sie die beiden Tabellen DEMO_J und BMX_J zusammen und entfernen Sie alle Spalten, die Sie für diese Aufgabe nicht brauchen. Ersetzen Sie numerisch kodierte Faktoren durch Text (also z.B. "male" statt 1 bei Gender). Die Tabellen mit den Bedeutungen der Codes finden Sie auf der Webseite in den Doc-Dateien neben den xpt-Dateien.

Berechnen Sie nun, wie schon in der vorigen Hausaufgabe, die durchschnittliche Körpergröße
der erwachsenen Probanden aus der NHANES-Studie, nun aufgeschlüsselt nach Ethnie und Geschlecht.
...
In dieser Aufgabe verwenden wir ausgedachte Daten, die aber in einer Form sind, wie sie sie Ihnen im Labor sicher noch begegnen werden.
...
Erstellen Sie bitte ein Quarto-Dokument mit Ihrem Code und Ihren Lösungen und laden Sie dieses als HTML-Datei auf Moodle hoch. Denken Sie daran, sicherzustellen, dass die Abbildungen in die HTML-Datei integriert werden, wie [hier](standalone.html) erklärt.